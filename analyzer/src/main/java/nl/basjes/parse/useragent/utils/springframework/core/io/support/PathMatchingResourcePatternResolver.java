...
import nl.basjes.parse.useragent.utils.springframework.core.io.DefaultResourceLoader;
import nl.basjes.parse.useragent.utils.springframework.core.io.FileSystemResource;
...
import nl.basjes.parse.useragent.utils.springframework.util.PathMatcher;
...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
    ...
    protected Resource[] findAllClassPathResources(String location) throws IOException {
        ...
        if (LOGGER.isTraceEnabled()) {
            LOGGER.trace("Resolved classpath location [{}] to resources {}", location, result);
        }
                ...
                for (URL url : ((URLClassLoader) classLoader).getURLs()) {
                    try {
                        ...
                        if (LOGGER.isDebugEnabled()) {
                            LOGGER.debug("Cannot search for matching files underneath [{}] because it cannot be converted to a valid 'jar:' URL: {}", url, ex.getMessage());
                        }
                    }
                }
            } catch (Exception ex) {
        ...
        try {
            ...
            for (String path : StringUtils.delimitedListToStringArray(
                ...
                try {
                    ...
                    if (LOGGER.isDebugEnabled()) {
                        LOGGER.debug("Cannot search for matching files underneath [{}] because it cannot be converted to a valid 'jar:' URL: {}", path, ex.getMessage());
                    }
                }
            }
        } catch (Exception ex) {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Failed to evaluate 'java.class.path' manifest entries: {}", ex.getMessage());
            }
        }
    }
    ...
    protected Set<Resource> doFindPathMatchingFileResources(Resource rootDirResource, String subPattern)
        ...
        try {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Cannot search for matching files underneath {} in the file system: {}", rootDirResource, ex.getMessage());
            }
            return Collections.emptySet();
        } catch (Exception ex) {
    ...
    protected Set<File> retrieveMatchingFiles(File rootDir, String pattern) throws IOException {
        ...
        if (!rootDir.canRead()) {
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("Skipping search for matching files underneath directory [{}] because the application is not allowed to read the directory", rootDir.getAbsolutePath());
            }
            return Collections.emptySet();
        }
    ...
    protected void doRetrieveMatchingFiles(String fullPattern, File dir, Set<File> result) throws IOException {
        if (LOGGER.isTraceEnabled()) {
            LOGGER.trace("Searching directory [{}] for files matching pattern [{}]", dir.getAbsolutePath(), fullPattern);
        }
    ...
    private static class PatternVirtualFileVisitor implements InvocationHandler {
        ...
        PatternVirtualFileVisitor(String rootPath, String subPattern, PathMatcher pathMatcher) {
